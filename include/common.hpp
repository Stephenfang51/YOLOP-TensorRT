#ifndef YOLOP_COMMON_H_
#define YOLOP_COMMON_H_
...
ILayer* hardSwish(INetworkDefinition *network, ITensor& input, int out_channel) {

    Weights constant_three{DataType::kFLOAT, nullptr, out_channel}; //the channel should be same with 
    float *add_ = new float[out_channel];
    for (unsigned int i = 0; i < out_channel; i++){
        add_[i] = 3.0f;
    };

    // float add_[1] = {3};    
    constant_three.values = add_; //#send  address of first element
    IConstantLayer* m_add = network->addConstant(Dims3{out_channel, 1, 1}, constant_three); //add a new constant layer for adding to input
    ILayer* after_add = network->addElementWise(input, *m_add->getOutput(0), ElementWiseOperation::kSUM);
    auto htanh = network->addActivation(*after_add->getOutput(0), ActivationType::kCLIP); //clip for min:0, max:6
    htanh->setAlpha(0); //min
    htanh->setBeta(6); //max

    Weights constant_six{DataType::kFLOAT, nullptr, out_channel};
    float *div_ = new float[out_channel];
    for (unsigned int i = 0; i < out_channel; i++){
        div_[i] = 6.0f;
    };

    // float div_[1] = {6};
    constant_six.values = div_;
    IConstantLayer* m_div = network->addConstant(Dims3{out_channel, 1, 1}, constant_six);
    ILayer* elem_div = network->addElementWise(*htanh->getOutput(0), *m_div->getOutput(0),ElementWiseOperation::kDIV);
    assert(elem_div);
    return elem_div;
}


//for yolop convBlock (conv + bn + x * (tanh(x+3, 6)/6)), the activation func is hardswish
ILayer* convBlock(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int outch, int ksize, int s, int g, std::string lname) {
    ...
    // auto hardswish = hardSwish(network, *bn1->getOutput(0), outch);
    // assert(hardswish);
...
// n:num of bottlenecks, g:conv groups
ILayer* bottleneckCSP(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int input_channel, int output_channel, int n, bool shortcut, int g, float e, std::string lname) {
    Weights emptywts{ DataType::kFLOAT, nullptr, 0 };
    /*
    n means how many bottleneck
    */
    int c_ = (int)((float)output_channel * e);
    ...
    auto leakyrelu = network->addActivation(*bn->getOutput(0), ActivationType::kLEAKY_RELU);
    leakyrelu ->setAlpha(0.1);

    auto cv4 = convBlock(network, weightMap, *leakyrelu->getOutput(0), output_channel, 1, 1, 1, lname + ".cv4");
    return cv4;
}
...
//used for check output of layers
void Dimension_checker(ITensor &input, const char* layer_name){
    Dims temp_dimension = input.getDimensions();
    std::cout << layer_name << " : ";
    std::cout << temp_dimension.d[0] << " " << temp_dimension.d[1] << " " << temp_dimension.d[2] << " " << temp_dimension.d[3] << "\n";
}

//three convBlocks, final outputchannel is 1/2 inputchannel
ILayer* multi_convBlocks(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int input_channel, int scale_factor, std::string lname) {
    int c_ = input_channel / scale_factor;
    auto first_convblock = convBlock(network, weightMap, input, c_, 1, 1, 1, lname + ".cv1");
    auto m_cv2 = convBlock(network, weightMap, *first_convblock->getOutput(0), c_, 1, 1, 1, lname + ".m.0.cv1");
    auto m_cv3 = convBlock(network, weightMap, *m_cv2->getOutput(0), c_, 3, 1, 1, lname + ".m.0.cv2");
    Weights emptywts{ DataType::kFLOAT, nullptr, 0 };
    auto con_cv3 = network->addConvolutionNd(*m_cv3->getOutput(0), c_, DimsHW{1, 1}, weightMap[lname+".cv3.weight"], emptywts);
    auto con_cv2 = network->addConvolutionNd(input, c_, DimsHW{1, 1} , weightMap[lname+".cv2.weight"], emptywts);
    ITensor* temp_cat_tensor[] = { con_cv3->getOutput(0), con_cv2->getOutput(0) };
    IConcatenationLayer* cat = network->addConcatenation(temp_cat_tensor, 2);
    ...
    lr->setAlpha(0.1);
    auto final_convblock = convBlock(network, weightMap, *lr->getOutput(0), c_*2, 1, 1, 1, lname + ".cv4");
    return final_convblock;
}


ILayer * addResize(INetworkDefinition*network,  ITensor&input, nvinfer1::Dims3 dims_chw){
    /*
    network
    input:last layer input
    dims_chw :expected output dimensions
    */
    // const float resize_scale[4] = {1, scale, scale};
    
    IResizeLayer *upsample = network->addResize(input);
    upsample->setResizeMode(ResizeMode::kNEAREST);
    upsample->setOutputDimensions(dims_chw);
    // upsample->setScales(resize_scale, 3);
